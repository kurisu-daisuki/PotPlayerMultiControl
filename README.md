...
- **全局热键**: `Ctrl+Alt+Space` 播放/暂停，`Ctrl+Alt+Home` 回到起始点，`Ctrl+Alt+←` 后退，`Ctrl+Alt+→` 快进，`Ctrl+Alt+↑` 显示全部，`Ctrl+Alt+↓` 最小化全部。
...
**使用说明**
- 启动应用后，窗口列表和命令栏默认折叠；点击「窗口列表」「命令栏」可展开查看。
- 点击“播放/暂停全部”或使用 `Ctrl+Alt+Space`，向每个 PotPlayer 窗口发送媒体播放/暂停。
- 点击“后退/快进”或使用 `Ctrl+Alt+←` / `Ctrl+Alt+→`，按当前时间跨度跳转；可在「时间跨度(秒)」中修改，默认 5 秒。
- 点击“回到起始点”或使用 `Ctrl+Alt+Home`，将全部视频跳到文件开头。
- 点击“显示全部”或使用 `Ctrl+Alt+↑`，还原全部 PotPlayer 窗口并将其置于最上层（置顶，不抢焦点）。
